...
export default {
    ...
    axios.post('https://api.swiftapp.ml',
      ...
      result.data.orders.forEach(function (order) {
        ...
        var orderItem = {
          ...
          orderProgress : Math.floor(Math.random() * (100 - 5 + 1) ) + 5,
        ...
        order.orderDetails.items.forEach(function (menuItem) {
          ...
          orderItem.items.push({
            ...
            menuItemProgress : Math.floor(Math.random() * (100 - 5 + 1) ) + 5
          });
        });
  ...
  increaseItemPercentage({ commit }, orderId, itemId, percentage){
    //update API here
